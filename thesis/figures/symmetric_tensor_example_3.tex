...
\begin{tikzpicture}
    ...
    \coordinate (tp1) at ($(cc)+(-.25,-.25)$) {};
    \draw[-<-=.7] (cc) -- (tp1) node[below] {$\alpha_1$};
    \coordinate (tp1) at ($(cc)+(.25,-.25)$) {};
    \draw[-<-=.7] (cc) -- (tp1) node[below] {$\alpha_2$};
    \coordinate (tp2) at ($(cc)+(0,.3535)$) {};
    \draw[->-=.7] (cc) -- (tp2) node[above] {$\alpha_3$};
    % \rTens{black}{1.5cm}{0.2cm}{(cc)}
\end{tikzpicture}
