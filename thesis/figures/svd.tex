...
\begin{tikzpicture}
    \coordinate (rw1) at (0,0);
    \coordinate (rw2) at (.7,0);
    \coordinate (rw3) at (0.35,0);
    \draw ($(rw1)+(-.25,0)$)--($(rw2)+(.25,0)$);
    ...
    \end{scope}
    \cTens{gray}{0.1}{(rw3)}
    \node[above] at (rw1) {$U_\pdag^\pdag$};
    \node[below] at (rw3) {$\Lambda_\pdag^\pdag$};
    \node[above] at (rw2) {$V^\dag_\pdag$};
\end{tikzpicture}
