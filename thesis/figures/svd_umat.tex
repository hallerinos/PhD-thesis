\documentclass[tikz]{standalone} % border={left bottom right top}
\input{./Tensors_2.tex}
\input{../definitions.tex}
\usepackage{braket}
\begin{document}

\tikzset{
    %Define standard arrow tip
    >=stealth'
}

\tikzset{
    tst/.style={
      thin, opacity=0.5,
      dashed
    },
    my-arc/.style={
      start angle=0, end angle=360,radius=0.4
    }
  }

\tikzset{
    tst/.style={
      thin, opacity=0.5,
      dashed
    },
    my-arc2/.style={
      start angle=0, end angle=360,radius=0.2
    }
  }

\definecolor{blue}{rgb}{0.2745098039, 0.5333333333, 0.9450980392}
\definecolor{red}{rgb}{0.9176470588, 0.2588235294, 0.2078431373}
\definecolor{yellos}{rgb}{0.9764705882, 0.7333333333, 0.1764705882}
\definecolor{green}{rgb}{0.2039215686, 0.662745098, 0.3176470588}


\begin{tikzpicture}
    \rTens{red!75!white}{2.65}{2.65}{(0,0)}
    \rTens{red}{1}{2.65}{(-.825,0)}
    \node at (0,0) {\color{white}$U$};

    % \node[above] at (0,-1.325) {\color{white}$n_y$};
    % \node[left] at (1.325,0) {\color{white}$n_y$};
    \draw[white, <->] (-1.325, -1.1) -- (1.325, -1.1) node[pos=.5, above] {\color{white}$n_y$};
    \draw[white, <->] (1.125, -1.325) -- (1.125, 1.325) node[pos=.7, left] {\color{white}$n_y$};

    \draw[white, <->] (-1.325, 1.1) -- (-0.325, 1.1) node[pos=.5, below] {\color{white}$m$};
\end{tikzpicture}


\end{document}
