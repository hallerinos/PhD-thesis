...
\end{align}
Although $\gamma_n$ is, in general, non-integrable, the phase is not allowed to fluctuate randomly in time, which is specified by direct substitution of \cref{eq:strange_gauge} in \cref{eq:time_evolution}
\begin{align}
...
If we would start from an $n$-variable parametrization, for which $C$ forms an $n$-dimensional path, $\gamma_n$ can be used to define a more general topological invariant, related to the homotopy class of the mapping between parameter and target space of the parametrization\footnote{
...
The eigenvectors have eigenvalues $\varepsilon_\pm = \pm k\hbar B/2$ and can be parametrized as
\begin{align}
