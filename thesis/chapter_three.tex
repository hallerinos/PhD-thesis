...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Periodic table of topological insulators}
...
Note that $i,j$ are generic index labels -- i.e. it may represent a combination of the position $x$ and additional ``internal quantum numbers'' $i\equiv(x,\alpha,\beta,\dots)$ such as the sublattice index of the SSH model.
...
On-site symmetries, as the name suggests, only act on the ``internal'' degrees of freedom, such that they can be factorized $\hat U=\prod_x\hat V_x$ with the same $\hat V_x$ on all lattice positions.
Please note that $x\neq i$, such that $\hat V_x$ acts nontrivially on the ``internal dimension'' of the tuple $i\equiv(x,\alpha,\beta,\dots)$.
...
\begin{align}
    ...
    \end{array}
\end{align}
In particular, this set of symmetries is exhaustive and spans the ten possible behaviors of the Hamiltonian~\cite{Chiu2016} which are presented in \cref{tab:symmetry_classes}.
Consider the case of time reversal invariance, there are three distinct classes:
(i) $H$ is not time reversal invariant, denoted by $T=0$, (ii) $H$ is time reversal invariant and $T$ squares to $+\mathbb1$ in which case we write $T=+$ and (iii) $H$ is time reversal invariant and $T$ squares to $-$, denoted by $T=-$~\cite{Chiu2016}.
...
\begin{table}
    ...
    \caption{The ten symmetry classes of topological insulators and superconductors, sorted by time reversal, particle-hole and chiral symmetry~\cite{Altland1997}, and to which values the unitary transformation of the symmetry squares to. A $0$ indicates the absence of symmetry while $\pm$ represents the outcome of the squared symmetry transformation (see text above). The mathematical sets indicate if a topological insulator / superconductor exists in a given space dimension ($0$ to $8$) and which values the topological invariant assumes.}
\end{table}
