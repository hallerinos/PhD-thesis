...
For gapped Hamiltonians, the topological distinction is through adiabatic deformations of the phase diagram by smoothly changing its parameters.
...
\todoil{This paragraph comes too short and imprecise.}
...
Our work in \cref{mcd1} does not build up on the classification of topological phases, but aims to provide a simple tool to extract the topological index, i.e. the winding number of quasi one-dimensional systems.
As is apparent in \cref{eq:winding_number}, a straightforward measurement of the invariant requires a full tomography of the momentum eigenstates, which is hard to do on a practical level.
We propose a scheme which performs the momentum integration implicitly in real space, and does not require a full tomography of the states.
It relies on a dynamical measurement of the density distribution followed by a local quench, which can be readily implemented in systems of atoms trapped in optical lattices.
