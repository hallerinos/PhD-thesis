...
\end{align}
In \cref{eq:bipartition_orthonormal_basis} I conveniently use the sum convention.
...
\end{align}
The additional subscript $i$ denotes an isolated action of $\hat X_i/\hat Y_i/\hat Z_i$ on the $i$'th spin of the system.
...
\begin{align}
    ...
    \end{pmatrix}
\end{align}
for the bulk $i\neq 1,L$ and a straightforward representation of the boundary tensors.
...
\begin{enumerate}
    ...
    \begin{align}
        f^{-1}\circ v(1,2) &= U^{[i_1]}\Lambda V^{\dag[i_{L}]},
        \\
        \includegraphics[valign=c]{figures/two_site_problem_2.pdf}
        &=
    ...
    \begin{align}
        f^{-1}\circ v(n+1,n+2) &= U^{[i_{n+1}]}\Lambda V^{\dag[i_{L-n}]},
        \\
        \includegraphics[valign=c]{figures/growing_4_4.pdf}
        &=
        \includegraphics[valign=c]{figures/growing_5.pdf}.
...
\begin{enumerate}
    ...
    \begin{align}
        f^{-1}\circ v(n,n+1) &= U^{[i_n]}\Lambda(n) V^{\dag[i_{n+1}]}\\
        \includegraphics[valign=c]{figures/growing_4_4.pdf}
        &=
        \includegraphics[valign=c]{figures/growing_5.pdf}
        .
...
\end{align}
The indication of the dependence on $m$ is reminiscent of the fact that $\varepsilon$ depends on the bond dimension of the MPS.
It can then be checked that the variance converges to machine precision by performing a so-called bond dimension scaling analysis~\cite{Hubig2018}.
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The projector $\hat P_{T\ket\psi}$ onto the tangent space of the MPS of $\ket\psi$ for a single site is written as
\begin{align}
    \hat P_{T\ket\psi} = \sum_{\ell=1}^L \hat P_{A(\ell)}\mathbb1_\ell \hat P_{B(\ell+1)} - \sum_{\ell=1}^{L-1}\hat P_L(\ell)\hat P_R(\ell+1)
\end{align}
...
Due to the gauge degree of freedom of the MPS we can always choose the tensors of partition $A/B$ left/right orthogonal, and as such find the trivial matrix representations of $P_{A(\ell)/B(\ell)}=\mathbb 1$.
Let me make this point more clear in the following set of equations:
\begin{align}
