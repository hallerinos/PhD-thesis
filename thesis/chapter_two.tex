...
One of the most crucial consequences of these simplifications is that quantitative deviations occur if the interactions are comparable to the kinetic bandwidth.
...
%
%
In the context of tensor networks, a tensor is understood as a multidimensional array containing complex elements.
Its rank is defined as the number of indices: scalar values are of rank zero, vectors of rank one and matrices of rank two\footnote{
    Note that this definition of ``rank'' differs from the usual understanding as image space dimension.
}.
...
\begin{figure}
    ...
    \caption{Different complexity classes for the same contraction, but different sequences. Assume each index has dimension $m$, then the green contractions are $O(m^4)$, but red is $O(m^5)$. Hence, sequence $1$ requires less multiplications compared to $2$ and is less prone to numerical truncation errors.}
...
The previous equation can be recast to
\begin{align}
...
\end{align}
in which $f^{-1}$ denotes the inverse of the tensor reshaping / leg fusion, and Einstein notation implies the tensor contraction with summed index $k$.
...
The number of singular values kept in the approximation is naturally linked to the accuracy of the numerical renormalization.
...
In total, we arrive at a compression rate
\begin{align}
...
Note that the $L$-dimensional array $C$ depends on all internal quantum numbers and is exponentially large in the system size $L$ and the dimension of the quantum numbers $i_j$.
...
\end{align}
Imagine a bipartition $A|B$ with $A(\ell)$ formed by the first $1,\dots,\ell$ quantum numbers and $B(\ell)$ containing the remaining ones $\ell+1,\dots,L$ ($L$ denotes the total number of sites).
A trace over the complement states of one of the partitions gives the reduced density matrix of the block, i.e.
\begin{align}
...
The threshold $m$ is directly related to approximating the reduced density matrix $\hat \rho_{A/B}$ with the $m$ most likely states in the Schmidt decomposition of a given bipartition $A/B$.
...
In this context, $-\ln(s^2_k)$ is understood as the information content (or surprisal) of the Schmidt state, and $S_1$ is simply the expectation value of surprisal.
Such a definition is understood intuitively: measuring a state of very low probability is surprising, whereas likely quantum states bear a low surprise.
...
%     S_1(\ell)
%     =
%     \lim_{\alpha\rightarrow1}\frac1{1-\alpha}\ln\tr\hat\rho_A^\alpha
%     =
%     -\lim_{\alpha\rightarrow1}\partial_\alpha\ln\sum_k s_k^{2\alpha}
%     =
%     -\lim_{\alpha\rightarrow1}\frac{\sum_k\partial_\alpha s_k^{2\alpha}}{\sum_k s_k^{2\alpha}}
%     =
%     -\lim_{\alpha\rightarrow1}\sum_k \partial_{\alpha}\re^{\alpha\ln s_k^2}
%     \\
%     =
%     -\lim_{\alpha\rightarrow1}\sum_k \re^{\alpha\ln s_k^2}\ln s_k^2
%     =
%     -\sum_k s_k^2\ln s_k^2
%     =
%     -\tr \hat\rho_A\ln\hat\rho_A.
% \end{align}
...
Note that the maximum entropy of an MPS is always restricted by the bond dimension $m$, i.e. $S_1(\ell)<\ln(m)$.
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Surprisingly, the phenomenon is independent of the distance between the particles and forms a unique form of correlations exclusively present in quantum systems.
...
A state $\ket{\psi}$ is called $N$-partite separable if and only if it can be written as a product of $N$ subsystems~\cite{Horodecki2009}
\begin{align}
...
The earlier example of a bipartite system is an element of the Bell basis, which are also called EPR states
\begin{align}
...
As such, the reduced density matrices represent mixed states and reveal no information about the subsystems, although the full state is still pure and therefore fully characterized.
...
In particular, the von Neumann entropy of a product state vanishes\footnote{This is actually a consequence of a much more general statement for open quantum systems and mixed states: the entropy of a subsystem can be greater than the entropy of the global system if and only if the state is entangled~\cite{Horodecki1994}.
The inequality for separable states can be expressed as $S(\hat\rho)-S(\hat\rho_{A/B})\geq0$ with $\hat\rho$ the density matrix of the mixed state and $\hat\rho_{A/B}$ the reduced density matrix of bipartition $A/B$.
...
Note that the reduced density matriy of the Bell states is $\hat\rho_{A/B}=\frac12\mathbb1$ and as such $S_1=\ln2$ which provides an upper limit for this two-partite system.
...
Assuming the local internal dimension is $D$, it assumes the upper bound of the entropy $S_1(\ell)=\ell\ln(D)$.
...
It is a remarkable result concerning the low-energy excitations of gapped and local one-dimensional Hamiltonians, for which the von Neumann entropy scales as the surface area of the bipartition.
Essentially, the works can be summarized as follows~\cite{Eisert2010}:
Consider a finite regular lattice of size $N$ and with spacing $a$, and a bipartition $1,2,\dots,\ell$ and $\ell+1,\ell+2,\dots,N$.
Then the von Neumann entropy of the physically relevant part (i.e. the low-energy/temperature subspace of $\HS$) scales as
\begin{align}
    S_1(\ell) &=
    \frac c{3b}
    \left\{
...
Although a general proof as in the case of 1D gapped Hamiltonians is presently not established, area laws have been proven for non-interacting and gapped models (both bosonic and fermionic), and have also been checked for various strongly correlated interacting systems~\cite{Eisert2010,Laflorencie2016}.
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The most straightforward example of a matrix product operator (MPO) is given through the Hamiltonian:
...
%
%
The variational ground state search in the context of MPS was initially formalized by the density matrix renormalization group (DMRG), introduced by Steven White in 1992~\cite{White1992}.
...
To find the ground state, we extremize the energy
\begin{align}
...
\begin{enumerate}
    ...
    \end{align}
    \item Compute the new lowest energy vector $\bm A_C$ according to \cref{eq:MPS_optimization}\footnote{At the boundaries of the system, ${\bm A}_C$ is effectively a rank-$3$ tensor and one of the isometries will necessarily be a rank-$2$ tensor under the decompositions schematized above.} and decompose it to two isometries and a matrix $\Lambda$ containing the singular values
    \begin{align}
...
One can identify a set of $2L-1$ independent differential equations
\begin{align}
...
A single integration step from $t\rightarrow t+\Delta t$ is achieved after a full sweeping cycle, and the error of the integration is more favorable $\mathcal O(\Delta t^3)$~\cite{Haegeman2016}.
...
For instance, for the Hubbard model with $N$ particles, a canonical infinite-temperature state is $\ket{\psi_0}=\hat C^{\dag N}\ket0$~\cite{Barthel2016} with
\begin{align}
...
A representation of a symmetry group element acts on the quantum states as
\begin{align}
...
Abelian symmetries have one-dimensional irreps in general, i.e.
\begin{align}
    W^\alpha(g) = \re^{\ri\varphi_\alpha(g)},
\end{align}
in which the phase $\varphi$ depends on the primary quantum number and the group element itself.
In this case, a secondary quantum number does not emerge~\cite{Silvi2019}.
...
It is possible to generate a bijection between the leg index value $i$ and symmetric link tuple $(\alpha,\gamma)$.
...
\end{align}
It is a necessity that $\alpha_3$ compensates the sum of charges $\alpha_1$ and $\alpha_2$.
