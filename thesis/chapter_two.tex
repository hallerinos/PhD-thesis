...
Recall that any state can be expressed by a set of good quantum numbers, such that
\begin{align}
    \ket{\psi} = \sum_{i_1,i_2,\dots,i_n}C({i_1,i_2,\dots,i_n})\ket{i_1,i_2,\dots,i_n}
    \label{eq:generic_state}
\end{align}
and the collection of all $\ket{i_1,i_2,\dots,i_n}$ form a complete and orthonormal basis.
Note that the $n$-dimensional array $C$ depends on all internal quantum numbers and is thus exponentially large in the system size $n$ and the dimension of the quantum numbers $i_j$.
By the visual conventions, we may imagine the coefficients $C$ as a rank $n$ tensor, similar to the tensor $T$ discussed in \cref{eq:SVD_generalized}
\begin{align}
    C(i_1,i_2,\dots,i_{n-1},i_n) = C^{i_1,i_2,\dots,i_n} \equiv  \includegraphics[valign=c]{figures/arbitrary_wavefunction.pdf}.
\end{align}
This allows to apply the generalized SVD in a sequential manner:
Starting with the decomposition spanned by the bipartition of the first (last) leg, keeping the left (right) isometry and shifting the right (left) isometry to the adjacent leg position, is possible to decompose $C$ into a product of isometries with a single $S$ matrix in between two arbitrary leg positions $\ell,\ell+1$.
The necessary decomposition sequence is most easily reformulated in graphical notation
\begin{align}
    C^{i_1,i_2,\dots,i_n} \equiv \includegraphics[valign=c]{figures/arbitrary_wavefunction.pdf}
    =
...
\begin{align}
    C^{i_1,i_2,\dots,i_n} \equiv \includegraphics[valign=c]{figures/arbitrary_wavefunction_decomposed.pdf}
\end{align}
is called the canonical form of an MPS.
In other words, it represents the Schmidt decomposition of the state $\ket{\psi}$ in the subspaces $\HS_A$ and $\HS_B$ spanned by the first $\ell$ and last $n-\ell$ quantum numbers i.e.
\begin{align}
    \ket\psi = \sum_{\bm i} C^{\bm i}\ket{\bm i} = \sum_{k}s_{k}\ket{\psi_{A,k}}
    \ket{\psi_{B,k}}
\end{align}
...
\begin{align}
    \ket{\psi_{A,k}}
    &=
    \sum_{i_1,i_2,\dots,i_j}
    U^{i_1}_{k_1} U^{i_2}_{k_1,k_2}U^{i_3}_{k_2,k_3}\dots U^{i_j}_{k_{j-1},k}\ket{i_1,i_2,\dots,i_j},\\
    %
    \ket{\psi_{B,k}}
    &=
    \sum_{i_{j+1},i_{j+2},\dots,i_n}
    {V^\dag}^{i_{j+1}}_{k,k_{j+1}} {V^\dag}^{i_{j+2}}_{k_{j+1},k_{j+2}}{V^\dag}^{i_3}_{k_{j+2},k_{j+3}}\dots {V^\dag}^{i_n}_{k_{n-1}}\ket{i_{j+1},i_{j+2},\dots,i_n}.
\end{align}
...
\end{align}
Now imagine to compute the reduced density matrix of a bipartition formed by a block $A$ of the first $\ell$ quantum numbers and a block $B$ of the remaining ones (same as before).
...
Please note that $s_k$ refers to the $k$'th singular value of a particular bipartition $A(\ell)/B(n-\ell)$, which considers the first/last $\ell/n-\ell$ quantum numbers.
...
\begin{align}
    \hat o_q\ket\psi &= \sum_{i_1,i_2,\dots,i_q,\dots,i_n}\sum_{i_q'}o_{i_q',i_q} C^{i_1,i_2,\dots,i_q,\dots,i_n}\ket{i_1,i_2,\dots,i_{q-1},i_q',i_{q+1},\dots,i_n}
    \\
    &= \sum_{i_1,i_2,\dots,i_n}\tilde C^{i_1,i_2,\dots,i_n}\ket{i_1,i_2,\dots,i_n}
\end{align}
...
\begin{align}
    \tilde C^{i_1,i_2,\dots,i_n} = \sum_{i_q'}o_{i_q,i_q'}C^{i_1,i_2,\dots,i_q',\dots,i_n}.
\end{align}
...
\end{align}
Note that the Hamiltonian can be recast into a product of matrix operators of the form
\begin{align}
    \hat W^{[i]} =
    \begin{pmatrix}
...
%     \end{pmatrix},
% \end{align}
which is readily solved by two auxiliary boundary tensors
\begin{align}
    \hat W^{[0]} =
    \begin{pmatrix}
        \mathbb 1 & 0 & 0
...
Consider the way the energy expectation value is written in the MPS language: it is a sandwich of the product of $W$ operators (a local MPO representing the Hamiltonian) with the trial wavefunction.
...
\begin{align}
    \braket{\psi|\hat H|\psi} - \lambda\brlr{\braket{\psi|\psi}-1} = 0.
\end{align}
Now the MPS structure of $\ket\psi$ is exploited, and to proceed I decompose the state into the form
\begin{align}
    \ket\psi
    =
    \sum_{\bm i}\brlr{\prod_{k=1}^LT^{[k]}}\ket{\bm i}
    =
    \sum_{\bm i,\bm k}T^{i_1}_{k_1} T^{\i_2}_{k_1,k_2}\dots T^{\i_q}_{k_{q-1},k_q}\dots T^{\i_{L-1}}_{k_{L-2},k_{L-1}}T^{\i_L}_{k_{L-1}}\ket{\bm i}
    \\
    \equiv
    .
\end{align}
To cast this equation into a variational problem for the colored tensor at position $q$, it is now more convenient to move to the graphical notation.
The Lagrangian equation becomes
\begin{align}
    0 = \braket{\psi|\hat H|\psi} - \lambda(\braket{\psi|\psi}-1)\equiv,
\end{align}
on which the the gradient $\nabla_{{T^{[q]}}^*}$ is applied (the derivatives with respect to all entries of the red tensor).
In this trivial example, the application of the gradient corresponds to removing the red tensor (and the scalar $1$) from the previous equation and one arrives at the appealing result
\begin{align}
    = .
\end{align}
This can be better understood after reshaping the contracted all-gray tensors to matrices $h_{\rm eff.}$, $n_{\rm eff.}$ of dimension $m^2D\times m^2D$ and $m^2\times m^2$, respectively. The tensor $T^{[q]}$ is then a vector $v$ of dimension $m^2D$ which reduces the previous equation to
\begin{align}
    h_{\rm eff.}v = \lambda n_{\rm eff.}v,
\end{align}
a generalized eigenvalue problem for the vector $v$ than can be solved efficiently with libraries like ARPACK~\cite{Lehoucq1998}.
Note that the generalized eigenvalue problem can be reduced to a standard eigenvalue problem using the canonical form of MPS, in which case $n_{\rm eff}=\mathbb1$.
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%
