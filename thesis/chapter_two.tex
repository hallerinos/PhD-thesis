...
\end{align}
I now proceed by the application of a gradient on both sides of \cref{eq:lagrangian_multiplier}, in which case the elements correspond to the entries of the contraction of the three colored tensors, i.e.
\begin{align}
    \nabla(\ell,\ell+1)_{kn}\equiv\frac\partial{\partial\brlr{U^{i_\ell}_{k,l}S_l V^{\dag i_{\ell+1}}_{l,n}}^*}.
\end{align}
The application of the gradient corresponds to removing the three central tensors from the conjugate contraction (and of course the scalar $1$) in \cref{eq:lagrangian_multiplier_graphical}.
...
\end{align}
This can be better understood after reshaping $U^{[\ell]}\Lambda V^{\dag[\ell+1]}$ (the three colored tensors in \cref{eq:lagrangian_multiplier_graphical_2}) to a vector $v({\ell,\ell+1})$ of dimension $m^2D^2$ and the contracted network of the gray tensors to a matrix $h_{\rm eff.}(\ell,\ell+1)$ of dimension $m^2D^2\times m^2D^2$, such that
\begin{align}
    h_{\rm eff.}(\ell,\ell+1)v({\ell,\ell+1}) = \lambda_\psi v({\ell,\ell+1}).
\end{align}
The Lagrange multiplier $\lambda_\psi$ thus corresponds to the global energy of the MPS and the equations span a standard eigenvalue problem for the vector $v$ which can be solved efficiently for the end of the spectrum with libraries like ARPACK~\cite{Lehoucq1998}.
...
The aim now is to find the optimal vectors $v_{\ell,\ell+1}$ for all computational sites, which can be achieved by the growing and finite MPS algorithm.
\\

The growing MPS algorithm is now formalized as follows:
\begin{enumerate}
    \item Compute the lowest energy state of two sites and decompose it to $U^{[1]}\Lambda V^{\dag[2]}$
    \begin{align}
        =
    \end{align}
    \item Split the tensor by two and copy it to its center, growing the MPS from $2n$ to $2n+2$ sites
    \begin{align}
        \rightarrow
    \end{align}
    \item Compute the lowest energy tensor of the central two sites and continue with 2.
    \begin{align}
        \rightarrow
    \end{align}
\end{enumerate}
