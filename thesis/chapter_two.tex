...
Additionally, from now on I use the convention that an object $T^{[i_1,\dots,i_n]}$ is a tensor which depends on the collection of physical quantum numbers $\bm i$.
...
\begin{align}
    ...
    \end{pmatrix}.
\end{align}
The MPO representation $W$ of the Hamiltonian $\hat H$ is versatile -- any Hamiltonian can be recast into a set of matrix product operators that are upper triangular, even if the interactions are long-ranged or spacially dependent, e.g. in the case of disorder.
...
\begin{enumerate}
    ...
    Backevolve $C(n)$ according to \cref{eq:TDVP_integration_C}, then continue with step 3. (sweeping from left to right) or 4. (sweeping from right to left).
...
\end{enumerate}
Similarly to finite MPS, TDVP is performed in completed sequences of full sweeping cycles $n=1,2,\dots,L$ and then $n=L,L-1,\dots,1$.
...
Completing the left to right sequence ($n=L,L-1,\dots,1$) then constitutes to composing the integration with its adjoint, resulting in a second-order symmetric method~\cite{Haegeman2016}.
A single integration step from $t\rightarrow t+\Delta t$ is thus achieved after a full sweeping cycle, and the error of the integration is more favorable $\mathcal O(\Delta t^3)$.
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
