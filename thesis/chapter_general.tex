...
\begin{align}
    C_{i,j}=A_{ik}B_{kj}=\includegraphics[valign=c]{figures/MatrixMultiplication.pdf},
\end{align}
...
\begin{align}
    \tr\brlr{ABCDEF} = \includegraphics[valign=c]{figures/MatrixTrace.pdf}.
\end{align}
...
\begin{figure}
    ...
    \caption{Different complexity classes for the same contraction, but different sequences. Assume each index has dimension $m$, then the green contractions are $O(m^4)$, but red is $O(m^5)$. Hence, sequence $S_1$ is faster and more precise compared to $S_2$.}
    \label{fig:contraction_sequences}
\end{figure}
A triangle representation may make sense to indicate a tensor being an isometry.
For instance, consider the two isometries $U$, $V^\dag$ of a generic singular value decomposition (SVD) $M = U S V^\dag$: they contain the left- and right-singular vectors of $M$ and are thus semi-unitaries, i.e.
\begin{align}
...
\begin{align}
    U^\dag U^\pdag = \includegraphics[valign=c]{figures/right_isometry.pdf} = \includegraphics[valign=c]{figures/right_identity.pdf}\,,
    \qquad
    V^\dag V^\pdag = \includegraphics[valign=c]{figures/left_isometry.pdf} = \includegraphics[valign=c]{figures/left_identity.pdf}\,.
\end{align}
...
\begin{align}
    ...
    \equiv
    f^{-1}\circ U^\pdag_{(i_1,\dots,i_j),k}S^\pdag_{k}V^\dag_{k,(i_{j+1},\dots,i_n)},
    \\
    \overset{(ii)}{=}&
    ...
    \equiv
    U_{i_1,k}^{\pdag i_2,\dots,i_j}S^\pdag_{k}{V^\dag}^{i_{j+1},\dots,i_{n-1}}_{k,i_n},
...
In the equations, it is assumed that the applied SVD is compact: $S$ is a square and diagonal matrix containing the nonzero singular values, such that $S_{ij}\equiv S_i\delta_{ij}$.
\\
...
\end{figure}
If the singular/spectral values are ordered, a truncation of the $S$ matrix neglects subdominant left and right eigenvectors of the original object and as such approximates the original matrix.
...
The approximated picture yields a compression rate of roughly $19\%$, and visible differences to the original are negligible.
\\

The identical compression scheme can actually be applied to quantum states.
...
\begin{align}
    C(i_1,i_2,\dots,i_{n-1},i_n) \equiv C^{i_1,i_2,\dots,i_n} =  \includegraphics[valign=c]{figures/arbitrary_wavefunction.pdf}.
\end{align}
This allows to apply the generalized SVD in a sequential manner:
Starting with the decomposition spanned by the bipartition of the first (last) leg, keeping the left (right) isometry and shifting the right (left) isometry to the adjacent leg position, is possible to decompose $C$ into a product of isometries with a single $S$ matrix in between two arbitrary leg positions $j,j+1$.
This particular decomposition of a generic quantum state, i.e.
\begin{align}
    C^{i_1,i_2,\dots,i_n} = \includegraphics[valign=c]{figures/arbitrary_wavefunction_decomposed.pdf}
\end{align}
is called the canonical form of matrix product states.
In this notation, it is obvious that any state can be recast to the canonical form, and that MPS have a large degree of gauge freedom.
To be more precise, one may sandwich infinitely many identities written as unitary matrices which can rotate the local tensors, but leave the overall MPS invariant.
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
