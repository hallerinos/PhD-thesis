...
If we approach $P\rightarrow\infty$, the allowed energies are formed by the roots of $f$, which are given by the dominating term and yield discretized energies $E_{\infty,n_b}=(\hbar n_b\pi)^2/(2ma^2)$ spanned by integer numbers ($n_b=1,2,\dots$).
...
Straightforward evaluation of $\arccos(f)$ evaluates the crystal momentum $ka$ which is displayed in \cref{fig:kronig_penney_dispersion} (b).
\\
...
\end{align}
Starting from \cref{eq:kronig_penney_transcendental_equation_approx_bound}, we see that for any solution $\alpha a$ there are an infinite number of symmetric points in momentum space which fulfill the same equation, i.e. (a) $ka + 2n\pi$ and (b) $-ka+2n\pi$.
...
In other words (b) simply flips the direction of the propagating wave and establishes a mirror symmetry at the $ka=0$ axis which is a consequence of the system being invariant under time-reversal.
The properties (a) and (b) denote the so-called ``unfolding'' of the reduced scheme presented in \cref{fig:kronig_penney_dispersion} (b).
It uniquely relates a crystal momentum $n_b\pi\leq ka<(n_b+1)\pi$ to an energy value $n_b\pi\leq \alpha a<(n_b+1)\pi$ by introducing a band index $(n_b=0,1,\dots)$.
Here, the limit of free particles ($P=0$) is readily restored, since \cref{eq:kronig_penney_constants} vanishes for $\alpha a = ka$.
\begin{figure}
    \centering
...
In the limit $P\rightarrow\infty$, the energy $E_{\infty,n_b}$ takes discrete values and thus ultimately kills any motion -- the resulting waves are tightly bound to the potential minimum.
...
\begin{align}
    \varepsilon\approx\varepsilon_0 - t \cos(k a),
    \quad
    \varepsilon_0 = E_{\infty,n_b=1} - \frac{\pi^2 \hbar^4}{a^3 m^2 b V_0},
    \quad
    t = -\frac{\pi^2\hbar^4}{a^3 m^2 bV_0}.
\end{align}
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
