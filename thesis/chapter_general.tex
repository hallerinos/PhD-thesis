...
In order to read-out the Bloch weights I conveniently factor a crystal momentum from the wavefunction
\begin{align}
...
The wavefunctions are supposed to be smooth at the boundaries\footnote{I hereby use the notation of limits from above or below, i.e. $f(0^\pm)=\lim_{\epsilon\rightarrow0}f(\epsilon)$ for $\epsilon>0$.}, which results in
\begin{align}
...
\begin{align}
    M\coloneqq\begin{pmatrix}
        +\re^{-\tau\alpha b} & +\re^{+\tau\alpha b} &
        -\re^{-\tau'\beta  b} & -\re^{+\tau'\beta  b} \\
        %
        +\alpha\re^{-\tau\alpha b} & -\alpha\re^{+\tau\alpha b} &
        -\beta \re^{-\tau'\beta  b} & +\beta \re^{+\tau'\beta  b} \\
        %
        +\re^{(\tau\alpha-\ri k) a}  & +\re^{-(\tau\alpha+\ri k) a} &
        -1                      & -1                      \\
        %
        +(\tau\alpha-\ri k)\re^{(\tau\alpha-\ri k) a}    & -(\tau\alpha+\ri k)\re^{-(\tau\alpha+\ri k) a} &
        -(\tau'\beta-\ri k)                          & -(\tau'\beta +\ri k)                       \\
    \end{pmatrix},
\end{align}
...
\end{align}
which approximate \cref{eq:kronig_penney_transcendental_equation} in the limit of narrow and strong periodic potentials\footnote{This limit is actually equivalent to a potential composed by delta-distributions.}
\begin{align}
    f(\alpha a) = \cosh(\alpha\tau a) + \tau'^2\frac{P}{\alpha\tau a}\sinh(\alpha\tau a).
    \label{eq:kronig_penney_transcendental_equation_approx}
\end{align}
If we now assume bound states of the form $0<E<V_0$, we have $\tau^2=-1$ and $\tau'^2=+1$, such that \cref{eq:kronig_penney_transcendental_equation_approx} becomes
\begin{align}
    f(\alpha a) = \cos(\alpha a) + \frac{P}{\alpha a}\sin(\alpha a).
    \label{eq:kronig_penney_transcendental_equation_approx_bound}
\end{align}
The right-hand-side of \cref{eq:kronig_penney_transcendental_equation_approx_bound} is in general not bound to the interval $[-1,+1]$ spanned by $\cos(ak)$ and establishes values of $\alpha$ (thus, the square-root of the energy $E$) for which no real-valued momentum can be found (see \cref{fig:kronig_penney_transcendental}).
\begin{figure}
    \centering
...
\begin{figure}
    ...
    \caption{Shapes of the dispersion relation $(\alpha a/\pi)^2$ versus crystal momentum $ka/\pi$ given by \cref{eq:konig_penney_energies_bound_simplified}. Different opacity (transparent to colors) represent increasing values of $P\in\{0.1,1,5,20,50,1000\}$.}
