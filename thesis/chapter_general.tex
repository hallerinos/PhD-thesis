...
However, to get an idea of the form of the Bloch functions, we here consider the simplest scenario of a particle moving in a one-dimensional potential
\begin{align}
    V_{ae}(x) = V_0 \Theta\brlr{\frac a2+{\rm mod}_{b}{x}}\Theta\brlr{\frac a2-{\rm mod}_{b}{x}},\ V_0,a,b\in\mathds R
\end{align}
...
The solution of both regions is a superposition of two plain waves with momentum $\hbar^2\alpha^2 = 2mE$ and $\hbar^2\beta^2 = 2m(E-V_0)$ and to read-out the Bloch factors I conveniently factor a crystal momentum
\begin{align}
...
\end{align}
which need to be smooth for all repetitions $n\in 0,\dots,L-1$,
\begin{align}
    \psi_{\alpha k}^{\rm (i)}(na+0^-)=\psi_{\beta k}^{\rm (ii)}(na+0^+),
    \quad
    {\psi_{\alpha k}^{\rm (i)}}'(na+0^-)={\psi_{\beta k}^{\rm (ii)}}'(na+0^+),
    \\
    \psi_{\alpha k}^{\rm (i)}(na+b+0^-)=\psi_{\beta k}^{\rm (ii)}(na+b+0^+),
    \quad
    {\psi_{\alpha k}^{\rm (i)}}'(na+b+0^-)={\psi_{\beta k}^{\rm (ii)}}'(na+b+0^+),
\end{align}
and the Bloch functions are periodic
\begin{align}
    u_{\alpha k}^{\rm (i)}(x+b) = u_{\alpha k}^{\rm (i)}(x),
    \quad
    {u_{\alpha k}^{\rm (i)}}'(x+b) = {u_{\alpha k}^{\rm (i)}}'(x).
\end{align}
%
...
To get an idea, we can solve the Schrödinger equation for a particularly easy model -- the Konig-Penney model, for which the potential is 
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
