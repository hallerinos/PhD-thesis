...
For convenience, let me proceed with the assumption that the two-point correlations are of the form $\braket{h({\bf x})h({\bf x'})} = \frac K2 C(|{\bf x}-{\bf x'}|)\rd l$ such that the action is rewritten to
\begin{align}
...
\end{align}
The constant $\gamma$ encodes the function $C(r)\approx\gamma\delta(r)$.
This approximation is actually highly non-trivial and needs further discussion.
For the employed momentum restrictions in \cref{eq:rg_hsq}, one would obtain here $C(r) = J_0(\Lambda r)$ with asymptotic expression $J_0(\Lambda r)\approx\sqrt{2/(\pi \Lambda r)}\cos(\Lambda r-\pi/4)$.
...
\begin{align}
    \int_{0}^{\Lambda}\rd p \rightarrow \int_{0}^\infty\rd p\frac{\Lambda^{n}}{p^{n}+\Lambda^{n}},
    \quad
    n\in\mathds N,
...
\begin{figure}
    \centering
    \subfigure[]{\includegraphics[width=.49\textwidth]{figures/Crs_cutoff.pdf}}
    \subfigure[]{\includegraphics[width=.49\textwidth]{figures/Crs.pdf}}
    \caption{TO BE REPLACED. Panel (a) shows the chosen cutoff for the integral expression in \cref{eq:integral_cutoff}, which results in the approximation of $C(r)$ plotted in (b).}
    \label{fig:rg_cutoff}
\end{figure}
...
\begin{align}
    ...
    \\
    =
    \frac K2\int_{0}^\infty\rd p J_0(p r)p^{n-1}\frac{n\Lambda^{n-1}}{\left(\Lambda^n+p^n\right)^2}\rd\Lambda + O(\rd \Lambda^2).
    \label{eq:rg_hxhxpr}
\end{align}
...
\end{align}
For the case $n=2$, the integral of the $h$-fields evaluates to
\begin{align}
    \braket{h({\bf x}),h(\bf x')}_h = \frac K2 r K_1(\Lambda r).
\end{align}
This concludes the analytic derivation of $C(r)=rK_1(\Lambda r)$, which is presented in \cref{fig:rg_cutoff}.
It is noticeable that a smooth cutoff is useful to confine the fast-field correlations into the interval $\Lambda x < 1$, which justifies $C(r)\approx\gamma\delta(r)$.
\\

The first $\cos$ term in \cref{eq:RG_second_order_approximation} did not exist before -- it is a new sine-Gordon type interaction with scaling dimension $D_h = K\beta^2$.
...
The second $\cos$ contribution in \cref{eq:RG_second_order_approximation} yields a renormalization of the quadratic part after the harmonic approximation (fully neglecting the constant which renormalizes the free energy)
\begin{align}
    -\frac12\braket{S^2_I[\phi_{\Lambda'}+h]}_{h,{\rm conn.}}
    \approx
    \frac{\gamma g^2K\beta^4}{16}\rd l\int\rd x\rd\tau\frac1{u^2}(\partial_
...
\begin{align}
    \frac1{uK'}=\frac1{uK}+\frac{\gamma g^2 K\beta^4\pi}{8 u^2}\rd l,
    \quad
    \frac u{K'}=\frac u{K}+\frac{\gamma g^2 K\beta^4\pi}8\rd l,
    \quad
    g' = g\brlr{1 + \commutator{2-\frac{\beta^2 K}4}\rd l},
    \\
    \Rightarrow
    K' = \brlr{\commutator{\frac{1}{uK}+\frac{\gamma g K\beta^4\pi}{8u^2}}\commutator{\frac{u}{K}+\frac{\gamma g K\beta^4\pi}{8}}}^{-1/2}
    =
    K-\frac{\pi \gamma \beta^4 g^2 K^3}{8u}\rd l + O(\rd l^2).
\end{align}
...
\begin{align}
    \frac{\rd K}{\rd l} = -\frac{\pi \gamma \beta^4 g^2 K^3}{8u},
    \quad
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Abelian symmetries}
\label{sec:abelian_symmetries}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
