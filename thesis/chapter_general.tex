...
The first order term can be rewritten to
\begin{align}
...
The $\cos$ term in (\cref{eq:rg_int_2}) yields a renormalization of the quadratic part after the harmonic approximation (fully neglecting the constant which renormalizes the free energy)
\begin{align}
    -\frac12\braket{S^2_I[\phi_{\Lambda'}+h]}_{h,{\rm conn.}}
    \approx
...
To become familar with the compression scheme, consider the way a generic density matrix
\begin{align}
    \hat\rho = \ket{\psi}\bra{\psi} = \sum_{k,k'}s_ks_{k'}\ket{\psi_{A,k},\psi_{B,k}}\bra{\psi_{A,k'},\psi_{B,k'}}
\end{align}
is expressed in terms of a canonical MPS.
Imagine to compute the reduced density matrix of a bipartition after the $\ell$'th quantum number (same as before).
A trace over the complement states of the partition $A/B$ gives the reduced density matrix, i.e.
\begin{align}
...
Therefore, it is appropriate to denote this dependence also in the spectral values, i.e. $s_k = s_k(\ell)$.
If we now keep the $m$ largest singular values of any bipartition $A(\ell)$, $m$ is becomes a global property of the MPS and thus deserves a special name, usually called the bond dimension.
\\

Density matrices are naturally related to the generalized Gibbs entropy for quantum states
\begin{align}
    S_1 = \tr(\hat\rho_A\log\hat\rho_A) = \tr(\hat\rho_B\log\hat\rho_B) = \sum_k s^2_k\log(s^2_k)
\end{align}
...
\begin{align}
    S_\alpha = \frac{1}{1-\alpha}\tr\log\rho^\alpha
\end{align}
...
\end{align}
In conclusion, using MPS techniques in the canonical form provides immediate access to the information content (or surprise) of a given bipartition.
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
A state $\ket{\psi_1,\psi_2\dots,\psi_N}$ is called fully $N$-partite separable iff. it can be written as~\cite{Horodecki1994}
\begin{align}
    \ket{\psi_1,\dots,\psi_N} = \ket{\psi_1}\otimes\ket{\psi_2}\otimes\dots\otimes\ket{\psi_N}.
\end{align}
In order to check this condition, it is sufficient to determine if the reduced density matrices $\rho_i$ of the elementary subsystems are pure [$\tr\rho_i^2=1$].
On the contrary, the state is genuinely $N$-partite entangled, iff. all bipartite
