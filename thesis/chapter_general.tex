...
\begin{figure}
    ...
    \caption{Example Wannier functions of the first two bands for a periodic potential of the form $V_{ae}(x)=V_0\sin(2\pi/ax)$ for different potential depth.}
...
\end{align}
To be more precise, the Wannier basis is not unique: every momentum-resolved Bloch function can be multiplied with a complex phase without changing its properties.
This naturally provides a gauge freedom to optimize the localization of the Wannier functions and thus the determination of a maximally localized Wannier function is actually a pretty involved task.
However, to get an idea how Wannier functions may look like, let us here consider a periodic potential of the form
\begin{align}
...
To obtain the Wannier functions displayed in \cref{fig:tight_binding_wanniers}, we gauge every Bloch function to be purely real at $x=0$, resulting in the example wave functions presented in Fig.~\cref{fig:tight_binding_wanniers}.
