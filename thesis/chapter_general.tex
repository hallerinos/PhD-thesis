...
\begin{figure}
    ...
    \caption{Panel (a) shows the chosen cutoff $f_n(p,\Lambda)$ with $n\in\{2,4,6,8,\infty\}$ for the integral expression in \cref{eq:integral_cutoff}, which results in the approximation of $C_n(r)$ plotted in (b). Panel (c) highlights the exponential sharp function $C_2(r)=\Lambda rK_1(r\Lambda)$ compared to the sharp cutoff result $C_\infty(r)=J_0(\Lambda r)$.}
    \label{fig:rg_cutoff}
\end{figure}
...
Therefore, the integration of $C_2(r)$ can be confined to a small interval $r<\xi$ in which $\xi\sim1/\Lambda$ is a small length scale comparable with the lattice spacing.
\\

In order to utilize the confinement of $C_2$, it is beneficial to introduce relative coordinates ${\bf R} = 1/2({\bf x}+{\bf x'})$ and ${\bf r} = 1/2({\bf x}-{\bf x'})$.
The integral can then be rewritten to
\begin{align}
    -\frac12\braket{S^2_I[\phi_{\Lambda'}+h]}_{h,{\rm conn.}}
    =
    \frac{g^2K\beta^2}8\rd l\int\rd^2R\int_{0}^\xi\rd^2r
    C_2(r)
    \left(
    ...
    \\
    \approx
    \frac{\xi^2 g^2K\beta^2}8\rd l\int\rd^2R
    \left(
        \cos\commutator{2\beta\phi_{\Lambda'}({\bf R})}
        -
        \cos\commutator{\beta\partial_{\bf R}\phi_{\Lambda'}({\bf R})\xi)}
    \right)
    \label{eq:rg_int_rel_coords}
\end{align}
in which I use the fact that $\lim_{r\rightarrow0}C_2(\Lambda r)=1$.
The $\cos$ term in \cref{eq:rg_int_1} did not exist before -- it is a new sine-Gordon type interaction with scaling dimension $D_h = K\beta^2$.
...
\begin{align}
    -\frac12\braket{S^2_I[\phi_{\Lambda'}+h]}_{h,{\rm conn.}}
    \approx
    \frac{\xi^4 g^2K\beta^4}{16}\rd l\int\rd x\rd\tau\frac1{u^2}(\partial_
...
\begin{align}
    \frac1{uK'}=\frac1{uK}+\frac{\xi^4 g^2 K\beta^4\pi}{8 u^2}\rd l,
    \quad
    \frac u{K'}=\frac u{K}+\frac{\xi^4 g^2 K\beta^4\pi}8\rd l,
    \quad
    g' = g\brlr{1 + \commutator{2-\frac{\beta^2 K}4}\rd l},
    \\
    \Rightarrow
    K' = \brlr{\commutator{\frac{1}{uK}+\frac{\xi^4 g K\beta^4\pi}{8u^2}}\commutator{\frac{u}{K}+\frac{\xi^4 g K\beta^4\pi}{8}}}^{-1/2}
    =
    K-\frac{\pi \xi^4 \beta^4 g^2 K^3}{8u}\rd l + O(\rd l^2).
\end{align}
...
\begin{align}
    \frac{\rd K}{\rd l} = -\frac{\pi \xi^4 \beta^4 g^2 K^3}{8u},
    \quad
