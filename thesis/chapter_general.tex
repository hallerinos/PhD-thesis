...
\begin{align}
    P_{i,j}^{c}\equiv\includegraphics[valign=c]{figures/tensor_picture.pdf}.
\end{align}
The position of the pixel is encoded by the indices $i,j$ and $c$ represents a leg containing the RGB-values (e.g., $c\in\{1,2,3\}\equiv\{r,g,b\}$).
...
There exist three different bipartitions: (a) $P_{(i,j),c}$, (b) $P_{(i,c),j}$ and (c) $P_{i,(j,c)}$.
...
The bipartition (a) targets a decomposition between $(i,j)$ and $c$ whereas (b) targets a decomposition between $i$ and $(j,c)$.
...
Truncation of the singular values implicitly truncates the physically important rows and columns to be taken into consideration.
...
\begin{figure}
    ...
    \caption{(a) Uncompressed picture of a beach with dimensions $n_y\times n_x=3024\times2276$. (b) Singular values of the matrix $P_{i,(jc)}$. (c) Keeping the $250$ largest singular values still yields a good approximation of the uncompressed picture and requires only $19.3\%$ of the original storage space.}
    \label{fig:fock_spaces}
\end{figure}
%
%
