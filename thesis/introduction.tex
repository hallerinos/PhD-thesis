...
%
In lectures on elementary quantum mechanics, the subject is initially formulated in ``first quan\-ti\-zation'', where the wave function is explicitly solved using differential equations.
This approach can be both beautiful and frustrating at the same time as even simple problems are rather involved, if not even impossible to solve analytically.
At a later stage one gets acquainted with ``second quantization'', which is more powerful in that respect since it relies on operator actions.
The nomenclature of ``first'' and ``second'' is quite confusing -- there is no such thing as two alternative ways of quantization -- it's just two equivalent and consistent ways of describing the same theory.
...
We then illustrate the treatment of interactions on top of a quadratic/non-interacting Hamiltonian using crucial tools in the understanding of many-body quantum matter.
We introduce the concept of Luttinger liquids (see \cref{sec:tomonaga_LL}), from which we derive the quantum field theory satisfying the algebra of a quantum harmonic oscillator.
...
A common perturbation potentially melting a Luttinger liquid phase is a sine-Gordon interaction that appears naturally in systems of coupled wires (see~\cref{sec:LL_with_spin}).
In order to treat such higher-order interactions analytically, we introduce the concept of renormalization group theory (see \cref{sec:renormalization_group_theory}), and study if and when the gapless phase evolves to a gapped, massive field theory.
...
This algorithm is quite similar to the time evolution presented in \cref{sec:time_evolution} and it can also be used to simulate finite temperature systems, presented in \cref{sec:finite_temperature}.
...
In \cref{part:results}, I summarize my original contributions to our published works and present them in a thematic manner.
