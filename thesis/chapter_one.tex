...
\end{align}
\todo{Comment on the overcompleteness of the $r$-states, or not?}
...
For this purpose, we assume the problem of the single-particle Hamiltonian $\hat H_0$ to be fully solved, such that the Bloch states $\psi_{\alpha{\bm k}}$ are determined, diagonalize $\hat H_0$ and have energy eigenvalues $\varepsilon_{\alpha{\bm k}}$.
...
If the matrix $T$ spans a higher dimensional lattice, complex transition amplitudes can give rise to nontrivial fluxes penetrating the lattice which may alter the underlying physics.
...
\todoil{Is this true for all number of flavors?}
...
\footnote{
    ...
    From this analog picture, and for all practical purposes, one may borrow the identities $\braket{\bm\phi(t)|\bm\pi(t')}=\frac1{2\pi}\exp\brlr{\ri\bm\phi(t)\bm\pi(t')}$ and $\mathbb1=\brlr{\prod_{j=1}^{N_L}\int\rd\phi_j}\ket{\phi_1,\dots,\phi_j}\bra{\phi_1,\dots,\phi_j}=\int\rd\bm\phi\ket{\bm\phi}\bra{\bm\phi}=$ (same for $\hat{\bm\pi}$).
    ...
    For a far more rigorous derivation of the functional path integral using coherent state representations, I refer to \cite{AltlandSimons2010}.
}.
...
\begin{align}
    \re^{-\ri\hat H\Delta t} = \re^{-\ri\Delta t\hat H(\bm\phi)}\re^{-\ri\Delta t\hat H(\bm\pi)}+\mathcal O(\Delta t^2) = \re^{-\ri\Delta t\hat H(\bm\pi)}\re^{-\ri\Delta t\hat H(\bm\phi)}+\mathcal O(\Delta t^2).
\end{align}
The error in the above is well under control by taking $N$ sufficiently large, and vanishes in the continuum limit.
Playing the same game as before, the insertion of $\mathbb1=\int\rd\bm\pi\ket{\bm\pi}\bra{\bm\pi}$ in between the two exponentials yields the approximate form of the transfer matrix, i.e.
\begin{align}
    T_{{\bm\phi}',{\bm\phi}} =
    \int\rd\bm\pi
    \braket{{\bm\phi}'|\re^{-\ri\Delta t\hat H(\frac{\bm\phi+\bm\phi'}2)}\ket{\bm\pi}\bra{\bm\pi}\re^{-\ri\Delta t\hat H(\bm\pi)}|{\bm\phi}} + \mathcal O(\Delta t^2),
    \\
    % =
    % \int\rd\bm\pi\braket{{\bm\phi}'|\bm\pi}\braket{\bm\pi|{\bm\phi}}\re^{-\ri\Delta t H(\frac{\bm\phi+\bm\phi'}2,\bm\pi)} + \mathcal O(\Delta t^2)
    =
    \int\frac{\rd\bm\pi}{2\pi}\re^{\ri\bm\pi(\bm\phi'-\bm\phi)}\re^{-\ri\Delta t H(\frac{\bm\phi+\bm\phi'}2,\bm\pi)} + \mathcal O(\Delta t^2).
\end{align}
Note that we succeeded to replace the operator form of the Hamiltonian by its scalar eigenvalues, such that the full time evolution kernel evaluates to
\begin{align}
    U(x_f,x_i,T) =
    \int{\rd\{\bm\phi\}}\int{\rd\{\bm\pi\}}
    \re^{\ri\Delta t\sum_{n=1}^{N}\brlr{\bm\pi_n\frac{\bm\phi_{n}-\bm\phi_{n-1}}{\Delta t}-H(\frac{\bm\phi+\bm\phi'}2,\bm\pi)}},
\end{align}
in which I conveniently use $\bm\phi_{0/N}\equiv \bm\phi_{i/f}$, $\rd\{\bm\phi\}=\prod_{n=1}^N\rd\bm\phi$ and $\rd\{\bm\pi\}=\prod_{n=1}^N\rd\bm\pi/2\pi$.
Now we carefully take the continuum limit of the time-slices $N\rightarrow\infty$: the sum in the exponential becomes a time integration $\Delta t\sum_{n=0}^{N-1}\rightarrow\int\rd t$ and the difference in the field configurations a derivative $(\phi_{n+1}-\phi_n)/\Delta t\rightarrow\dot\phi$.
The result is the astonishing identity
\begin{align}
    U(x_f,x_i,t) \rightarrow \int_{\bm\phi_i}^{\bm\phi_f}\rD({\bm\phi,\bm\pi})\re^{\ri S(\bm\phi,\bm\pi)}
\end{align}
with the definition of the Euclidian action
\begin{align}
    S = \int_{0}^{t}\rd t\bm\pi\dot{\bm\phi}-H(\bm\phi,\bm\pi) = \int\rd tL(\bm\phi,\dot{\bm\phi})
\end{align}
and the integration measure
\begin{align}
    \int\rD({\bm\phi,\bm\pi}) = \lim_{N\rightarrow\infty}\int\prod_{n=1}^{N-1}\rd\bm\phi_n\int\prod_{n=1}^{N}\frac{\rd\bm\pi_n}{2\pi}.
\end{align}

The Lagrangian of the Hamiltonian \cref{eq:ll_hamiltonian} is given by
\begin{align}
    L = \int\rd x\frac 12\brlr{\frac{(\partial_t\hat\phi)^2}{u'}-u'(\partial_x\hat\phi)^2}
    \label{eq:kg_lagrangian}
\end{align}
in which, for convenience, $u'=u/\pi$ is a rescaled velocity and $K$ is absorbed in the definition of fields.
The equations of motion reduce to
\begin{align}
    \partial_t\frac{\partial L}{\partial\hat\pi}+\partial_x\frac{\partial L}{\partial(\partial_x\hat\phi)} = 0,
    \quad
    \partial_t^2\hat\phi  = {u'}^2\partial_x^2\hat\phi.
    \label{eq:KG_equations_of_motion}
\end{align}
The conjugate field to $\hat\phi$ is, not surprisingly, the original field $\hat\pi_\phi\equiv\hat\pi=\partial_x\hat\theta$ which satisfies
\begin{align}
...
\begin{align}
    ...
    0.
\end{align}
The two fields $\varphi_{R/L}$ are thus chiral and depend on $t\mp u'x$, respectively.
...
We now move to imaginary time $\tau=\ri t$, resulting in the action
\begin{align}
    S = -\frac1{2}\int\rd\tau\rd x\brlr{ \frac{(\partial_\tau\phi)^2}{u'} + u'(\partial_x\phi)^2}
\end{align}
...
\begin{align}
    Z_\phi = \int\rD_\phi \re^{-S[\phi]} = \int\rD_\phi \exp\brlr{-\int\frac{\rd k\rd\omega}{2\pi}\phi(-{\bm k})\commutator{\frac{w^2+(u'k)^2}{u'}}\phi({\bm k})}
\end{align}
With the definition of $G^{-1}({\bm k})=\frac{\omega^2+(u'k)^2}{2\pi}$ and an additional source term $-\int\rd^2k\eta(-{\bm k})\phi({\bm k})$, the partition function is transformed to
\begin{align}
...
This motivates the further study of such terms, and in order to tackle it it will be beneficial to move into the realm of path integrals and renormalization group theory.
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
In order to utilize the confinement of $C_2$, it is beneficial to introduce relative coordinates ${\bm R} = 1/2({\bm x}+{\bm x'})$ and ${\bm r} = {\bm x}-{\bm x'}$.
