...
The method is often referred to as Abelian bosonization and provides a mechanism to decouple interacting fermions in terms of non-interacting bosonic degrees of freedom.
...
The Umklapp scattering of spinless fermions can be rewritten according to
\begin{align}
    \sum_{k,k',q}V(q)
...
\end{align}
which proofs that Umklapp terms are up to a sign equivalent to backscattering terms in case of indiscernible particles.
...
It describes the fractionalization of particles into two distinct quasiparticles, called spinons with zero charge and spin 1/2 and chargons with charge minus one and no spin.
...
The second observation regards the presence of a non-quadratic term for $g_{1\perp}\neq0$.
Terms of the type
\begin{align}
    \hat O_{\rm sG}(\beta,\hat f) = g\int\rd x\cos(\beta\hat f)
\end{align}
are called sine-Gordon terms of the field $\hat f$ and are potentially ``relevant'' for a free Luttinger liquid.
...
\begin{align}
    {\Delta^*}\propto\re^{-l^*}=\brlr{\frac{g(0)}{g^*}}^{1/(2-D_g)}.
\end{align}
