...
\end{align}
The determinant is to be understood as the product of all eigenvalues of the operator $A$.
Although the constant of proportionality $(2\pi)^N$ is formally divergent in the continuum limit, it does not affect the definition of averages obtained from derivatives of the integrals, i.e.
\begin{align}
...
\end{align}
In which the (possibly divergent) terms arising from the explicit Gaussian integration are absorbed into the definition of $\rD$.
...
\end{align}
such that the path integral becomes a quantum mechanical partition function\footnote{
...
Traditionally, the BKT transition was first encountered in 2D classical systems, but by now it is well known that (1+1)D quantum systems are represented by partition functions which correspond to 2D classical systems (see also end of \cref{sec:properties_of_real_scalar_fields_and_their_correlations}).
...
\todoil{Missing assumptions according to Matteo?}
...
\end{align}
The $\cos$ term in \cref{eq:rg_int_1} did not exist in the original Hamiltonian\footnote{
...
} -- it is a new sine-Gordon type interaction with higher scaling dimension $D_h = K\beta^2$.
