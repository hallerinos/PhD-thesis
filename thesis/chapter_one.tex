...
In particular, the famously quoted Peierls' substitution approximates the effect of a uniform magnetic field, in which case the Hamiltonian follows the principle of minimal coupling
\begin{align}
...
\begin{align}
    \hat H_0
    &= \sum_k \hbar^2\brlr{\varepsilon_F \pm \frac{k_F}{m}(k\mp k_F) + \mathcal{O}([k\mp k_F]^2)}\hat n_k
    \\
    &= \sum_q \hbar^2\brlr{\varepsilon_F + \frac{k_F}{m}q\brlr{\hat n_{q+k_F}-\hat n_{q-k_F}} + \mathcal{O}(q^2)}
...
There is an indirect reasoning through Schur's lemma: if two operators $\hat H$ and $\hat H'$ have identical commutation relations with all $\{c^\pdag_\alpha,c^\dag_\alpha\}$, then the two operators are equal up to an overall constant, which in this case can be interpreted as a chemical potential.
...
A rigorous derivation of the field operators in terms of the local creation and annihilation operators is particularly clearly presented in several excellent books on the topic, e.g.~\cite{Bruus2004,Giamarchi2003,Gogolin2004}.
...
\end{align}
in which I conveniently use $\bm\phi_{0/N}\equiv \bm\phi_{i/f}$, $\rd\{\bm\phi\}=\prod_{n=1}^{N-1}\rd\bm\phi_n$ and $\rd\{\bm\pi\}=\prod_{n=1}^N\rd\bm\pi_n/2\pi$.
...
\begin{align}
    ...
    ,\quad
    S = -\frac1{2\pi u}\int\rd\tau\rd x(\partial_\tau\phi)^2 - V(\phi).
\end{align}
...
To apply the statements for the interacting theory, only the fields must be rescaled $\phi\rightarrow\phi/\sqrt K$ and $\theta\rightarrow\theta\sqrt K$, i.e.
\begin{align}
...
\end{align}
The Umklapp terms require special attention in this case:
...
Consider now that the energy gap scales similarly as the coupling amplitudes, such that an estimate during the RG flow is given by $\Delta(l)=\re^{l}\Delta(0)$.
...
For a sharp momentum cutoff, one would obtain here $C(r) = J_0(\Lambda r)$ with asymptotic expression $J_0(\Lambda r)\approx\sqrt{2/(\pi \Lambda r)}\cos(\Lambda r-\pi/4)$.
...
Therefore, the integration of $C_2(r)$ can be confined to a small interval $r<\alpha$ in which $\alpha\sim2\pi/\Lambda=a$ is a small length scale comparable with the lattice spacing $a$.
...
Note that $(x+2)^3\approx8$, such that $\rd x/\rd l = -y^2$ and $\rd y/\rd l=-xy$ describes the effective RG flow close to the Luttinger liquid fix point at $x=0$ and $y=0$.

In particular, we obtain four scenarios (assuming positive couplings $y>0$ in general):
...
(iii) In case of $y>|x|$, the coupling $y$ first flows towards smaller couplings, but then towards infinite $y$, independently on the value of $x$.
...
To characterize the trajectories of the flow chart, one must notice that $x\rd x = y\rd y$ which describes the solutions $c = x^2-y^2$ with $c$ constant.
The conserved quantity $c$ thus describes hyperbolas cutting the $x$ and $y$ axis for $c>0$ and $c<0$, respectively (cf. \cref{fig:bkt_flow_equations} (b)).
...
We can thus linearize $c=b^2(\lambda_c-\lambda)$ in the vicinity of the critical line, in which $\lambda$ encodes an internal parameter of the microscopic Hamiltonian that drives the phase transition.
...
\end{align}
An estimate of the correlation length in the system after the RG flow is then given by
\begin{align}
    \xi\propto a(l^*) = a\re^{l^*} = a\re^{\frac{\pi}{2b\sqrt{\lambda-\lambda_c}}},
\end{align}
in which $a\equiv a(0)$ is the lattice spacing.
Any attempt to numerically simulate systems close to situation (a) thus requires system lengths in excess of the exponentially divergent correlation length\footnote{In case of matrix product states, which I depict in \cref{ch:matrix_product_states}, the limitation is based on the Ansatz itself, which is efficient for systems with a short correlation length only.}.
