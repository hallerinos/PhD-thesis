...
The present chapter is inspired by a number of excellent books and lectures such as~\cite{AshcroftMermin1978,AltlandSimons2010,BruusFlensberg2004,Czycholl2016,FetterWalecka2003,Giamarchi2003,Rizzi2016,Burrello2020,negele1988quantum} extending the basic aspects of quantum mechanics to a modern way of understanding quantum field theory in general.
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The two-particle wave function can be represented by an inner product
\begin{align}
...
\end{align}
and span what is known as the (anti-)commutation relation
\begin{align}
...
Let us provide the operator form which act on a single particle of the full $N$-particle state (usually dubbed ``one-body operator'').
...
As we will see in the next section, Bloch waves can be used to define a complete set of tightly localized ``Wannier'' states that allow to introduce effective ``tight binding'' models in which the coupling parameters of the model are implicitly defined through overlap integrals of the Bloch functions.
...
To simplify the problem, we can consider two different regions, i.e. (i) a free particle $-\frac{\hbar^2}{2m}\partial_x^2\psi_{\rm (i)}(x) = E\psi_{\rm (i)}(x)$ and (ii) a particle moving in a constant potential $-\frac{\hbar^2}{2m}\partial_x^2\psi_{\rm (ii)}(x) = (E-V_0)\psi_{\rm (ii)}(x)$.
...
\end{align}
% This concludes the pedagogic recap of the Kronig-Penney model, and we proceed by giving a more generic recipe to solve arbitrary periodic potentials.
...
Here, we assume the problem of the single-particle Hamiltonian $\hat H_0$ to be fully solved, such that the Bloch states $\psi_{\alpha{\bm k}}$ diagonalize $\hat H_0$ and have energy eigenvalues $\varepsilon_{\alpha{\bm k}}$.
...
% Without going into detail about optimizing Wannier functions, we want to present a basic visualization of these localized states.
...
In the Kronig-Penney model, this corresponds to $V_0\gg \hbar^2/(mab)$ with particle mass $m$, lattice constant $a$ and potential width $b\ll a$.
...
%     .
% \end{align}
Expanding to first order in $B=|\bm B|$, the transition elements are dressed with a complex phase factor and result in
\begin{align}
...
It is important to note that the phase space of these studies is largely inaccessible by a realistic vector potential~\cite{Alexandrov1991,Julen2014}, and other methods like Raman-assisted tunneling~\cite{Garcia2012} must be used to achieve the desired complex phase modulation of the tunneling elements.
...
One of the most successful description of electrons in solids is band theory -- based on screened many-body interactions described by effective one-body potentials.
...
\end{align}
This modification demonstrates that $g_1$ terms are up to a sign equivalent to backscattering terms in case of indiscernible particles.
...
\begin{align}
    ...
    \sigma_\tau v_F \hbar q\hat\rho_{q,\tau}
\end{align}
and we conclude our previous statement $\hat H_0' = \hat H_0 + \mu$ with an irrelevant constant $\mu$.
...
The field $\partial_x\hat\phi$ can be interpreted as the ``position offset'' from the equilibrium, which in this case translates to density fluctuations around the equilibrium density $n=k_F/\pi$.
...
On a more intuitive level, the identity above shares a fundamental equivalence to the Jordan-Wigner transformation~\cite{Jordan1928}.
The exact identity
\begin{align}
...
% \begin{align}
%     \braket{\phi(x)\phi(0)} \approx -\frac K2\ln(x),
%     \quad
%     \braket{\theta(x)\theta(0)} \approx -\frac 1{2K}\ln(x),
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Correlation functions are essential to probe many-body phases on a practical level, and we evaluate a number of such observables throughout all of our works.
...
\end{align}
% Before we proceed, we must first comment a bit about the form of \cref{eq:field_source}.
...
Now for the second order correction -- the first term in the connected correlation function evaluates to
\begin{align}
...
The disconnected part is evaluated using the identity \mbox{$2\cos(a)\cos(b)=\cos(a+b)+\cos(a-b)$} and $(1-\alpha\rd l)^2 = 1-2\alpha\rd l + O(\rd l^2)$ and reads
\begin{align}
...
The second $\cos$ term (\cref{eq:rg_int_2}) yields a renormalization of the quadratic part,
\begin{align}
    -\frac12\braket{S^2_I[\phi_{\Lambda'}+h]}_{h,{\rm conn.}}
    \approx
...
However, MPS provides a possibility to extrapolate the results to both infinite system sizes and correlation lengths, which overcomes the related issues and allows to extract quantitative results with great success.
% \footnote{To be explicit here, through a finite system size scaling, followed by a finite bond dimension scaling, which controls the effective correlation length captured by the MPS approximation.}
