...
\end{align}
in which I conveniently use $\bm\phi_{0/N}\equiv \bm\phi_{i/f}$, $\rd\{\bm\phi\}=\prod_{n=1}^N\rd\bm\phi_n$ and $\rd\{\bm\pi\}=\prod_{n=1}^N\rd\bm\pi_n/2\pi$.
...
More importantly, the (arbitrary!) vector $\bm j$ acts as a ``source'' term to define an expectation value, i.e.
\begin{align}
    \braket{v_mv_n}\equiv\partial^2_{j_m,j_n}\int\rd\bm v\re^{-\frac12\bm v^T\bm A\bm v + \bm j^T \bm v}\Bigg|_{\bm j=0}
    =
...
\end{align}
Application of \cref{eq:multi_gaussian} and taking the continuum limits yields the path integral
\begin{align}
    U(x_f,x_i,T)
    =
...
We now imploy a so-called ``Wick rotation'' $\tau=-\ri t$, resulting in the ``Euclidian'' action
\begin{align}
    S = -\frac1{2\pi}\int\rd\tau\rd x\brlr{ \frac{(\partial_\tau\phi)^2}{u} + u(\partial_x\phi)^2}
\end{align}
such that the path integral becomes a quantum mechanical partition function
\footnote{
    Recall that $Z=\tr(\exp(-\beta \hat H))$, which equals a trace of the time evolution operator at imaginary time $-\ri\beta$, for which the derivation (in the employed case of real and scalar fields) is straightforward.
}
\begin{align}
    Z = \int\rD[\phi]\re^{-S[\phi]}.
\end{align}
The partition function is readily recast into frequency and momentum space (using the notation ${\bm k}=(\omega, k)$)
\begin{align}
    Z = \int\rD_\phi \re^{-S[\phi]} = \int\rD_\phi \exp\brlr{-\int\frac{\rd k\rd\omega}{2\pi}\phi(-{\bm k})\commutator{\frac{w^2+(uk)^2}{u}}\phi({\bm k})}
\end{align}
with the definition of the Green's function $G^{-1}({\bm k})=\frac{\omega^2+(uk)^2}{2\pi}$.
Similarly to \cref{eq:multi_source} an additional source term $-\int\rd^2k\eta(-{\bm k})\phi({\bm k})$ implemented in the partition function, i.e.
\begin{align}
    Z[\eta] = \int\rD_\phi \exp\brlr{-\frac12\int\rd k\rd\omega\phi(-{\bm k})G^{-1}({\bm k})\phi({\bm k}) -\int\rd k\rd\omega\eta(-{\bm k})\phi({\bm k})},
    \label{eq:partition_with_source}
\end{align}
leads to the expectation values defined by a second functional derivative
\begin{align}
    \braket{\phi({\bm k})\phi(-{\bm k})}=\frac{\delta^2}{\delta(\eta({\bm k}))\delta(\eta(-{\bm k}))}\frac{Z[\eta]}{Z[0]}\Bigg|_{\eta=0}.
\end{align}
Note that we conveniently get rid of the (possibly divergent) constants through $Z[0]^{-1}$.
...
Through the substitution $\eta(x) = \ri\sum_{k}b_k\delta(x-x_k)$ in \cref{eq:partition_with_source}, we arrive at the appealing result
\begin{align}
...
To make the statements valid for the interacting theory, only the fields must be rescaled $\phi\rightarrow\phi/\sqrt K$ and $\theta\rightarrow\theta\sqrt K$, i.e.
\begin{align}
...
I thus arrived at the asymptotic approximations for the Green's function in \cref{eq:greens_1,eq:greens_2} and a general formula to evaluate all kinds of vertex operators in \cref{eq:expectation_value_exponential_fields}.
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The $\cos$ term in (\cref{eq:rg_int_2}) yields a renormalization of the quadratic part after the harmonic approximation in polar coordinates $\bm r=(u(\tau-\tau'),x-x')^T$ and $r=\sqrt{(x-x')^2+u^2(\tau-\tau')^2}$
\begin{align}
    -\frac12\braket{S^2_I[\phi_{\Lambda'}+h]}_{h,{\rm conn.}}
    \approx
